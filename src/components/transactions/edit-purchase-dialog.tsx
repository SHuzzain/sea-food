...
import { updatePurchase } from "@/app/(app)/purchase/actions";
...
function toLines(items: EditablePurchase["items"]): Line[] {
  return items.length
    ? items.map((item) => ({
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    startTransition(() => {
      void (async () => {
        ...
        if (!result.ok) {
          setError(result.error ?? "Unable to update purchase.");
          return;
        }
        onOpenChange(false);
        router.refresh();
        ...
        <form className="space-y-4" onSubmit={submit}>
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <div className="space-y-2">
              ...
              <Input id="edit-purchase-date" type="date" value={purchaseDate} onChange={(event) => setPurchaseDate(event.target.value)} />
            </div>
          </div>
              ...
              return (
                <div key={line.key} className="space-y-3 rounded-md border p-3">
                  ...
                  <div className="grid grid-cols-2 gap-3 sm:grid-cols-3">
                    <div className="space-y-2">
                      <Label htmlFor={`edit-purchase-kg-${line.key}`}>Kg</Label>
                      <Input
                        id={`edit-purchase-kg-${line.key}`}
                        inputMode="decimal"
                        type="number"
                        min="0"
                        step="0.001"
                    ...
                    <div className="space-y-2">
                      <Label htmlFor={`edit-purchase-rate-${line.key}`}>Rate</Label>
                      <Input
                        id={`edit-purchase-rate-${line.key}`}
                        inputMode="decimal"
                        type="number"
                        min="0"
                        step="0.01"
