...
import {
  ...
  supplierStatementWhatsAppMessage
} from "@/lib/documents/supplier-statement-pdf";
import { sharePdfWithWhatsApp } from "@/lib/documents/whatsapp";
import { formatRupee } from "@/lib/utils";

export function SupplierStatementActions({
  statement,
  profile = DEFAULT_BUSINESS_PROFILE
}: {
  statement: SupplierStatementDocument;
  profile?: BusinessProfile;
}) {
  const filename = `supplier-statement-${statement.supplier.name.toLowerCase().replace(/[^a-z0-9]+/g, "-")}.pdf`;

  async function downloadPdf() {
    const blob = await buildSupplierStatementPdf(statement, profile);
  ...
  async function shareWhatsApp() {
    ...
    await sharePdfWithWhatsApp({
      filename,
      blob,
      phone: statement.supplier.mobile,
      message: supplierStatementWhatsAppMessage(statement, profile)
    });
  }
    ...
    <div className="space-y-4">
      <div className="grid gap-3 sm:grid-cols-3">
        ...
        <div className="rounded-md bg-muted p-3">
          ...
          <p className="font-semibold">{formatRupee(statement.totalPurchases)}</p>
        </div>
